                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            <?php
                                                ...
                                                $statement = "`tbl_contact_info`";
                                                
                                                // Pagination End

                                                $tstimonInfodata = array();
                                                $tstimonInfoquery = "SELECT `contact_id`, `contact_name`, `contact_mail`, `contact_sub`, `contact_date`, `contact_platform`, `contact_browser` FROM {$statement} ORDER BY `contact_id` DESC LIMIT {$startpoint} , {$limit}";
                                                $tstimonInforesult = $dbconnect->query($tstimonInfoquery);
                                                if ($tstimonInforesult) {
                                                    while ($tstimonInforows = $tstimonInforesult->fetch_array(MYSQLI_ASSOC)) {
                                                        $tstimonInfodata[] = $tstimonInforows;
                                                    }
                                                    $tstimonInforesult->close();
                                                }                                
                                                foreach ($tstimonInfodata as $tstimonInfoRow) {
                                                    echo '
                                                        <tr>
                                                            <td>' . ucfirst($tstimonInfoRow['contact_name']) . '</td>
                                                            <td>' . $tstimonInfoRow['contact_mail'] . '</td>
                                                            <td class="text-truncate" style="max-width: 150px;">' . $tstimonInfoRow['contact_sub'] . '</td>
                                                            <td>' . forEmptyDate($tstimonInfoRow['contact_date']) . '</td>
                                                            <td>' . $tstimonInfoRow['contact_platform'] . '</td>
                                                            <td>' . $tstimonInfoRow['contact_browser'] . '</td>
                                                            <td><a class="btnn btn-outline-sxc" href="viewcontact.php?viewid='.encode($tstimonInfoRow['contact_id']).'"><i class="fa fa-pencil"></i></a></td>
                                                        </tr>
                                                    ';
                                                }
                                            ?>
