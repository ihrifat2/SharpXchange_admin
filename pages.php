            ...
            <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top " id="navigation-example">
                <div class="container-fluid">
                    <div class="navbar-wrapper">
                        <a class="navbar-brand" href="javascript:void(0)">Pages</a>
                    </div>
                                ...
                                <div class="card-body">
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            <?php
                                                $pagesInfodata = array();
                                                $pagesInfoquery = "SELECT `page_id`, `title`, `url`, `update_on` FROM `tbl_page_settings`";
                                                $pagesInforesult = $dbconnect->query($pagesInfoquery);
                                                if ($pagesInforesult) {
                                                    while ($pagesInforows = $pagesInforesult->fetch_array(MYSQLI_ASSOC)) {
                                                        $pagesInfodata[] = $pagesInforows;
                                                    }
                                                    $pagesInforesult->close();
                                                }                                
                                                foreach ($pagesInfodata as $pagesInfoRow) {
                                                    echo '
                                                        <tr>
                                                            <td>' . $pagesInfoRow['title'] . '</td>
                                                            <td>' . $pagesInfoRow['url'] . '</td>
                                                            <td>' . forEmptyDate($pagesInfoRow['update_on']) . '</td>
                                                            <td><a class="btnn btn-outline-sxc" href="pagesettings.php?pageid='. encode($pagesInfoRow['page_id']) .'"><i class="fa fa-pencil"></i></a></td>
                                                        </tr>
                                                    ';
                                                }
                                            ?>
